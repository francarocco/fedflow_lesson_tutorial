...
for split in splits:
    for i in range(num_clients): #working area by area
        ...
        model = cnn_definition(input_shape,output_shape)

        #network training
        print('***Starting training with cnn***',flush=True)
        model.fit(X_train,y_train, validation_data=(X_val, y_val), epochs=epochs, verbose=2)

        print('***Starting testing with cnn***',flush=True)
        ...
        path_model = f'{path_files}/models/local_cnn_{i}_split{split}.h5'
        model.save(path_model)

        path_forecasts = f'{path_files}/results/forecasts_local_cnn_{i}_split{split}.npy'
        np.save(path_forecasts,forecasts)

        path_AE = f'{path_files}/results/AE_local_cnn_{i}_split{split}.pkl'
        AE.to_pickle(path_AE)

        path_SE = f'{path_files}/results/SE_local_cnn_{i}_split{split}.pkl'
        SE.to_pickle(path_SE)
        
        path_APE = f'{path_files}/results/APE_local_cnn_{i}_split{split}.pkl'
