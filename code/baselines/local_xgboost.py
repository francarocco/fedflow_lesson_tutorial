...
for split in splits:
    for i in range(num_clients): #working area by area
        ...
        #network training
        print(f'***Starting training with XGBoost for client {i}***',flush=True)
        model.fit(X_train_reshaped,y_train,eval_set=[(X_val,y_val)])

        print('***Starting testing with XGBoost***',flush=True)
        ...
        path_model = f'{path_files}/models/local_xgboost_{i}_split{split}.h5'
        model.save(path_model)

        path_forecasts = f'{path_files}/results/forecasts_local_xgboost_{i}_split{split}.npy'
        np.save(path_forecasts,forecasts)

        path_AE = f'{path_files}/results/AE_local_xgboost_{i}_split{split}.pkl'
        AE.to_pickle(path_AE)

        path_SE = f'{path_files}/results/SE_local_xgboost_{i}_split{split}.pkl'
        SE.to_pickle(path_SE)
        
        path_APE = f'{path_files}/results/APE_local_xgboost_{i}_split{split}.pkl'
